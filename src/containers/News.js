...
class News extends React.Component {
  ...
  pagesArray() {
    ...
    for (let i = 0; i < Math.ceil(this.state.filteredNews.length / 6); i += 1) {
      temp.push(i)
    }
  ...
  chooseCategory = category => {
    if (category === this.props.story.content.categories[1].text) {
      ...
      this.setState({
        filteredNews: this.props.news.filter(
  ...
  selectPage = page => {
    if (page > 0 && page <= Math.ceil(this.state.filteredNews.length / 6)) {
  ...
  render() {
    ...
    const { selected_category, filteredNews, page_number } = this.state
      ...
      <div>
                    ...
                    return (
                      <div key={category.id}>
                        <SbEditable content={this.props.story}>
                          <div className='category_description paragraph--lead'>{content.no_news_available}</div>
                        </SbEditable>
                      </div>
                    )
                  }
        ...
        <div className='wrapper'>
          <div className='pagination__wrapper'>
            {Math.ceil(filteredNews.length / 6) > 1 ? (
